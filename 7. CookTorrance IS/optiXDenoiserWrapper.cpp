...
void OptiXDenoiserWrapper::denoise(CUDABuffer input_buffer, CUDABuffer normal_buffer, CUDABuffer albedo_buffer, float4** output, unsigned int frame_number)
{
...
void OptiXDenoiserWrapper::setup_ldr(OWLContext& owl_context, const vec2i& newSize)
{
    setup(owl_context, newSize, OPTIX_DENOISER_MODEL_KIND_LDR);
}

void OptiXDenoiserWrapper::setup_hdr(OWLContext& owl_context, const vec2i& newSize)
{
    setup(owl_context, newSize, OPTIX_DENOISER_MODEL_KIND_HDR);
}

void OptiXDenoiserWrapper::setup(OWLContext& owl_context, const vec2i& newSize, OptixDenoiserModelKind model_kind)
{
