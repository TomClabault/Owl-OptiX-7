...
OPTIX_RAYGEN_PROGRAM(ray_gen)()
{
    ...
    optixLaunchParams.accumulation_buffer[pixel_index] += ray_color;
    ...
    vec3f primary_hit_normal_remapped = abs(primary_hit_normal);
    float4 primary_hit_normal_float4 = make_float4(primary_hit_normal_remapped.x, primary_hit_normal_remapped.y, primary_hit_normal_remapped.z, 1.0f);
...
OPTIX_MISS_PROGRAM(miss)()
{
    ...
    vec3f ray_direction = optixGetWorldRayDirection();
#if HDR
    //This is because the EXR files we read with the TinyEXR
    //library end up flipped along the horizontal axis. And because
    //I didn't find an option in TinyEXR to Y-flip the image during
    //read, I'm flipping it here in the shader :D
